...
def deployed_app() -> Generator[DeployedKraph, None, None]:
    ...
    with setup:
        ...
        mikro_http_url = f"http://localhost:{setup.spec.find_service('kraph').get_port_for_internal(80).published}/graphql"
        mikro_ws_url = f"ws://localhost:{setup.spec.find_service('kraph').get_port_for_internal(80).published}/graphql"
