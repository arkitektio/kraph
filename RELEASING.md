...
`kraph` ships as a PyPI package (`kraph`). Versioning is automated by
...
| `feat:` | minor | `feat: add node accessor` |
...
| --- | --- | --- |
| `main` | stable `X.Y.Z` | the default install (`pip install kraph`) |
| `next` | prereleases `X.Y.Z-rc.N` | published as a **prerelease** — only reached via `pip install kraph --pre` or an exact pin |
| `N.x` (e.g. `0.x`) | maintenance `X.Y.Z` | published stable for an older major |

PyPI marks `…-rc.N` versions as prereleases, so a plain `pip install kraph`
...
`jhnnsrs/kraph:${KRAPH_SERVICE_TAG:-latest}`, and `integration.yaml` sets the same
default. Every branch is therefore tested against `:latest`, the released line.

To test against another published tag, set the repository variable `KRAPH_SERVICE_TAG`
...
main   ──0.5.1──(merge next)──> 1.0.0 -> 1.0.1 …    (stable PyPI)
          │ cut `0.x` from main HEAD *before* the 1.0.0 merge
0.x    ──0.5.1──> 0.5.2 -> 0.5.3 …                  (stable PyPI for v0)
```
...
```sh
pip install kraph --pre          # latest rc (or stable, whichever is newer)
pip install 'kraph==1.0.0-rc.1'  # pin a specific rc
```

Stable consumers (`pip install kraph`) are unaffected by the `next` channel.
